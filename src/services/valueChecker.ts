  ...
  if (imageUrls.length > 0) {
    ...
    if (authenticity.success && authenticity.result) {
      if (!authenticity.result.isAuthentic) {
        const rejectionReason = `Bildanalyse (OpenAI gpt-4o) hat ${imageUrls.length} Bilder geprüft und erkennt echte Fälschungsanzeichen oder klare Qualitätsmängel: ${authenticity.result.reasons.join(' | ')} (niedrigste confidence: ${authenticity.result.lowestConfidence}).`;
